...
//------------------`groups`----------------------
	...
	//----------------1c codes in contractors-----------------------
	
	public function getCustomerCodeInContractorById($customerid,$contractorid) {

            $stmt = $this->conn->prepare("
			SELECT `customercode` FROM `customer_code_in_contractor` WHERE  `customerid` = ? AND `contractorid` = ? ");
	...
	public function setCustomerCodeInContractor($customerid, $customercode, $contractorid){
		...
		if($count==0){
			$stmt = $this->conn->prepare("INSERT INTO customer_code_in_contractor (customerid, customercode, contractorid) values( ? , ? , ? )");
			$stmt->bind_param("isi", $customerid, $customercode, $contractorid);
			$stmt->execute();
			$stmt->close();
		}
	...
	public function getUserCodeInContractorById($userid,$contractorid) {

            $stmt = $this->conn->prepare("
			SELECT `usercode` FROM `user_code_in_contractor` WHERE  `userid` = ? AND `contractorid` = ? ");
	...
	public function getUserIdInContractorByCode($usercode,$contractorid) {

            $stmt = $this->conn->prepare("
			SELECT `userid` FROM `user_code_in_contractor` WHERE  `usercode` = ? AND `contractorid` = ? ");
	...
	public function setUserCodeInContractor($userid, $usercode, $contractorid){

		//Update status column on user_code_in_contractor table
		$stmt = $this->conn->prepare("UPDATE `user_code_in_contractor` SET `usercode` = ? WHERE ((`userid` = ?) AND (`contractorid` = ?))");
		$stmt->bind_param("sii", $usercode,$userid,$contractorid);
		$result = $stmt->execute();
		$count = $stmt->affected_rows;
		$stmt->close();

		if($count==0){
			$stmt = $this->conn->prepare("INSERT INTO user_code_in_contractor (userid, usercode, contractorid) values( ? , ? , ? )");
			$stmt->bind_param("isi", $userid, $usercode, $contractorid);
			$stmt->execute();
			$stmt->close();
		}
