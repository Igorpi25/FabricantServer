...
$app->post('/analytic_agent_kustuk/:agentid/groups/', 'authenticate', function($agentid) use ($app) {
	...
	$akb_tea_done_groups_count=0;
	$akb_coffee_done_groups_count=0;
	...
	foreach($groups as $group){		
		...
		$group_mml_tea_products_count=array();
		$group_mml_coffee_products_count=array();
		...
		$group["akb_tea_summ"]=$tea_summ;
		$group["akb_coffee_summ"]=$coffee_summ;
		$group["akb_coffee_done"]=$coffee_summ>$coffee_akb_summ;
		$group["akb_tea_done"]=$tea_summ>$tea_akb_summ;
		...
		//Для общей АКБ по всем группам
		if($group["akb_tea_done"])$akb_tea_done_groups_count++;
		if($group["akb_coffee_done"])$akb_coffee_done_groups_count++;
	...
	if ($groups) {
		...
		//Количество магазинов где АКБ выполнен (чай/кофе соответственно)
		$response["akb_tea_done_groups_count"] = $akb_tea_done_groups_count;
		$response["akb_coffee_done_groups_count"] = $akb_coffee_done_groups_count;
